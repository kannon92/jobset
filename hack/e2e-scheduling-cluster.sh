...
export KUSTOMIZE="$(cd "$PWD" && realpath "${KUSTOMIZE:-$PWD/bin/kustomize}")"
export KIND="$(cd "$PWD" && realpath "${KIND:-$PWD/bin/kind}")"
...
# group is present. Uses a namespaced image tag and reuses an existing
