...
All pods within that PodGroup reference the same IMEX channel claim, enabling
multi-node GPU communication over NVLink.
...
the template, and all pods in that PodGroup share it. Pod-level GPU claims continue to
work independently. This resolves the need for Job-level ResourceClaimTemplates
without requiring changes to the core Job API.
