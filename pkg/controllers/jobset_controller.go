...
func (r *JobSetReconciler) reconcile(ctx context.Context, js *jobset.JobSet, updateStatusOpts *statusUpdateOpts) (ctrl.Result, error) {
	...
	if features.Enabled(features.WorkloadAwareScheduling) && js.Spec.Scheduling != nil {
		if jobSetSuspended(js) {
			...
			if err := r.reconcileWorkload(ctx, js); err != nil {
...
func constructJob(js *jobset.JobSet, rjob *jobset.ReplicatedJob, jobIdx int) *batchv1.Job {
	...
	if features.Enabled(features.WorkloadAwareScheduling) && js.Spec.Scheduling != nil {
		...
		if useTopLevelGang(js.Spec.Scheduling) && !hasSequencedStartup(js) {
			job.Annotations[SchedulingGroupTemplateNameKey] = js.Name
			pgName = js.Name
		} else {
			job.Annotations[SchedulingGroupTemplateNameKey] = rjob.Name
