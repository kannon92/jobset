...
func TestResolveLeafPolicy(t *testing.T) {
	tests := map[string]struct {
		rjob                     *jobset.ReplicatedJob
		leafPolicy               *jobset.ReplicatedJobSchedulingPolicy
		globalScheduling         *jobset.JobSetScheduling
		ignoreGlobalGangMinCount bool
		wantGang                 bool
		wantBasic                bool
		wantMin                  int32
	}{
		...
		"explicit Basic policy from leaf": {
			rjob: &jobset.ReplicatedJob{
				Name:     "driver",
				Replicas: 1,
				Template: batchv1.JobTemplateSpec{
		...
		"explicit Gang without minCount defaults to computed": {
			rjob: &jobset.ReplicatedJob{
				Name:     "workers",
				Replicas: 2,
				Template: batchv1.JobTemplateSpec{
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			policy := resolveLeafPolicy(tc.rjob, tc.leafPolicy, tc.globalScheduling, tc.ignoreGlobalGangMinCount)
			if tc.wantGang {
...
func TestBuildPodGroupTemplate(t *testing.T) {
	tests := map[string]struct {
		rjob                     *jobset.ReplicatedJob
		leafPolicy               *jobset.ReplicatedJobSchedulingPolicy
		globalScheduling         *jobset.JobSetScheduling
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			template := buildPodGroupTemplate(tc.rjob, tc.leafPolicy, tc.globalScheduling, tc.ignoreGlobalGangMinCount)
...
func TestBuildTopLevelGangTemplate(t *testing.T) {
	tests := map[string]struct {
		js             *jobset.JobSet
		wantName       string
		wantMinCount   int32
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			template := buildTopLevelGangTemplate(tc.js)
			assert.Equal(t, tc.wantName, template.Name)
			require.NotNil(t, template.SchedulingPolicy.Gang)
			assert.Equal(t, tc.wantMinCount, template.SchedulingPolicy.Gang.MinCount)
			if tc.hasConstraints {
