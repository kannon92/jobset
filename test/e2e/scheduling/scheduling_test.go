...
var _ = ginkgo.Describe("Workload-Aware Scheduling E2E", func() {
	...
	ginkgo.It("should preempt low-priority JobSet when high-priority JobSet needs resources", func() {
		...
		lowPC := &schedulingv1.PriorityClass{
			ObjectMeta: metav1.ObjectMeta{Name: "e2e-low-priority"},
			Value:      1,
		}
		highPC := &schedulingv1.PriorityClass{
			ObjectMeta: metav1.ObjectMeta{Name: "e2e-high-priority"},
			Value:      100000,
		}
		// Ignore AlreadyExists errors so the test is idempotent.
							...
							Spec: batchv1.JobSpec{
								...
								Template: corev1.PodTemplateSpec{
									Spec: corev1.PodSpec{
										...
										PriorityClassName:             "e2e-low-priority",
										RestartPolicy:                 corev1.RestartPolicyNever,
										Containers: []corev1.Container{{
											Name:    "worker",
											Image:   "docker.io/library/bash:latest",
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(pg.Spec.PriorityClassName).To(gomega.Equal("e2e-low-priority"))
		}, timeout, interval).Should(gomega.Succeed())
						...
						Template: batchv1.JobTemplateSpec{
							Spec: batchv1.JobSpec{
								...
								Template: corev1.PodTemplateSpec{
									Spec: corev1.PodSpec{
										TerminationGracePeriodSeconds: int64Ptr(0),
										PriorityClassName:             "e2e-high-priority",
										RestartPolicy:                 corev1.RestartPolicyNever,
										Containers: []corev1.Container{{
											Name:    "worker",
											Image:   "docker.io/library/bash:latest",
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(pg.Spec.DisruptionMode).NotTo(gomega.BeNil())
			g.Expect(pg.Spec.DisruptionMode.All).NotTo(gomega.BeNil())
			g.Expect(pg.Spec.PriorityClassName).To(gomega.Equal("e2e-high-priority"))
		}, timeout, interval).Should(gomega.Succeed())
