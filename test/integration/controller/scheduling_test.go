...
var _ = ginkgo.Describe("Workload-Aware Scheduling integration", func() {
	...
	ginkgo.It("should create a single PodGroup when top-level gang with no leaf overrides", func() {
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(pg.Spec.PodGroupTemplateRef).NotTo(gomega.BeNil())
			g.Expect(pg.Spec.PodGroupTemplateRef.Workload).NotTo(gomega.BeNil())
			g.Expect(pg.Spec.PodGroupTemplateRef.Workload.PodGroupTemplateName).To(gomega.Equal(js.Name))
		}, timeout, interval).Should(gomega.Succeed())
